# new_test/ — version corrigée du bot EDT

Reprend `edt_stri.py` + `test_local.py` avec les correctifs de la revue de code.
...
```bash
# Local (depuis la racine du dépôt)
python new_test/test_local.py                # télécharge le PDF puis traite
python new_test/test_local.py --no-download  # retraite le edt.pdf existant
python new_test/test_local.py --no-debug     # sans export des imagettes

# CI / production
python new_test/edt_stri.py --telecharger edt_nouveau.pdf
